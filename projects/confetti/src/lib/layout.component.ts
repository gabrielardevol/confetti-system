  ...
  template: `
    <div class="h-screen w-screen flex flex-col relative overflow-x-hidden">
      <nav class="h-0 overflow-visible sticky top-0 w-full z-10">
        <div class="bg-white border-b border-black" #navElement>
          <ng-content select="[nav]"></ng-content>
        </div>
      </nav>
      <aside class="border-r border-black sticky overflow-visible top-0 h-0 z-10" [ngClass]="menu ? '' : 'menu-hidden'" style="">
        <div
          [style.margin-top]="navbarSize + 'px'" class="relative">
          <div [style.height]="'calc(100vh - '+navbarSize + 'px'"
               class="absolute top-0 left-0 border-r border-black overflow-x-auto ">
